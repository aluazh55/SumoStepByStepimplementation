...
class AdaptiveController:
    ...
    #
    #   Пока дорога пустая — чередуем фазы каждые 20с.
    #   ИСПРАВЛЕНИЕ: Теперь переключение всегда идет через Yellow/All-Red
    #   для соблюдения норм безопасности (защита "невидимых" участников движения).
    #
    def _handle_idle(self, cs: int, cw: int):
        has_cars = cs > 0 or cw > 0

        # Машины появились, но пешеходный/минимальный цикл ещё не завершён — ждём
        ...
        # Время текущего цикла вышло
        if self.timer >= IDLE_CYCLE_TIME:
            if not has_cars:
                # Дорога пустая — ВСЁ РАВНО запускаем стандартный переход
                # Это гарантирует безопасность, если датчик кого-то не заметил
                return self._start_transition(
                    self._opposite_phase(),
                    "Idle rotation (Safety first)"
                )

            # Есть машины — выбираем фазу, где их больше, и идем в переход
            desired = self._choose_best_phase(cs, cw)
            if desired != self.current_phase:
                return self._start_transition(desired, "Idle -> switching to cars")
            else:
                # Машины на текущей фазе — активируем рабочий режим
